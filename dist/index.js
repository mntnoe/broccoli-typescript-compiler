...
function createParseConfigHost(workingPath, input) {
    ...
    function readDirectory(rootDir, extensions, excludes, includes, depth) {
        return ts.matchFiles(rootDir, extensions, excludes, includes, useCaseSensitiveFileNames, workingPath, depth, getFileSystemEntries);
    }
    ...
    if (resolution.isDirectory()) {
        if (resolution.isInput()) {
            ...
            if (resolution.isMerged()) {
                for (var other in this$1.readdir(resolution.canonicalPath).directories) {
...
function normalizeOptions(options, inputPath) {
    var workingPath = toAbsolutePath(options.workingPath === undefined ? inputPath : options.workingPath);
    var rootPath = options.rootPath === undefined ? workingPath : toAbsolutePath(options.rootPath, workingPath);
    var projectPath = options.projectPath === undefined ? rootPath : toAbsolutePath(options.projectPath, workingPath);
    var buildPath = options.buildPath === undefined ? undefined : toAbsolutePath(options.buildPath, workingPath);
...
var TypeScriptPlugin = (function (BroccoliPlugin$$1) {
    ...
    TypeScriptPlugin.prototype.build = function build () {
        ...
        if (!this.diagnosticHandler) {
            this.diagnosticHandler = new DiagnosticsHandlerImpl(this.normalizedOptions);
        ...
        if (!compiler) {
            compiler = this.compiler = new Compiler(toAbsolutePath(this.inputPaths[0]), toAbsolutePath(this.outputPath), this.normalizedOptions, this.diagnosticHandler);
        }
        compiler.compile();
        heimdall.stop(token);
    };
    TypeScriptPlugin.prototype.setDiagnosticWriter = function setDiagnosticWriter (write) {
